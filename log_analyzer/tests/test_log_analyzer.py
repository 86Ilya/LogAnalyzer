...
class TestLogAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.serialized_dict = '[{"count": 1, "time_avg": 0.39, "time_max": 0.39, "time_sum": 0.39,' \
                               ' "url": "/api/v2/banner/25019354", "time_med": 0.39, "time_perc": 74.5697896749522,' \
                               ' "count_perc": 50.0}, {"count": 1, "time_avg": 0.133, "time_max": 0.133,' \
                               ' "time_sum": 0.133, "url": "/api/1/photogenic_banners/list/?server_name=WIN7RB4",' \
                               ' "time_med": 0.133, "time_perc": 25.430210325047803, "count_perc": 50.0}]'
    ...
    def test_get_recent_log(self):
        log_name, log_date, log_type = log_analyzer.get_recent_log("./logs", log_analyzer.regexprs[
            "LOG_NAME_REGEXP"])
        self.assertEquals(log_name, 'nginx-access-ui.log-20170720')
        self.assertEquals(log_date, datetime.datetime(2017, 7, 20, 0, 0))
    ...
    def test_calculate_statistic(self):
        ...
        top_urls, report = log_analyzer.calculate_statistic(log_iter,
                                                            r'\"[A-Z]+\s(?P<url>[\S]+)\s.+\"\s(?P<time>\S+)$', 2, 0)
